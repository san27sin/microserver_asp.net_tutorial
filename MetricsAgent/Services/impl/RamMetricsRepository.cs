...
namespace MetricsAgent.Services.impl
{
    public class RamMetricsRepository : IRamMetricsRepository
    {
        ...
        public void Create(RamMetrics item)
        {
            ...
            cmd.CommandText = "INSERT INTO rammetrics(value,time) VALUES(@value,@time)";//параметр помечается @
        ...
        public void Delete(int id)
        {
            ...
            cmd.CommandText = "DELETE FROM rammetrics WHERE id=@id";//where указывает условия удаления
        ...
        public RamMetrics GetById(int id)
        {
            ...
            using var cmd = new SQLiteCommand(connection);
            //прописываем запрос на получение всех данных
            cmd.CommandText = "SELECT * FROM rammetrics WHERE id=@id";//по айди
            using (SQLiteDataReader reader = cmd.ExecuteReader())
            {
                //Пока есть что читать - читаем
